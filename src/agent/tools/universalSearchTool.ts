    ...
    try {
      ...
      // Specific search types
        ...
        if (response.success && response.data) {
          ...
          results = response.data.filter((r: any) => r.TypeId === 2);
        }
      }
        ...
        if (companyResponse.success && companyResponse.data) {
          ...
          if (hierarchyResponse.success && hierarchyResponse.data) {
            results = hierarchyResponse.data.map((h: any) => h.resource);
          }
        }
      }
      ...
      if (!includeInactive) {
        results = results.filter((r: any) => r.Active);
      }
      ...
      const formattedResults = limitedResults.map((resource: any) => {
